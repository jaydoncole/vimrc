 ...
 * @version $Date: 2018/04/12 13:22:54 $
 */
abstract class Globals implements ClassResolver {
	...
	/**
	 * Read-only FQN of the autoload function "phplint_autoload".
	...
	/**
	 * The phplint_autoload() function; NULL=still not found.
	 * @var Function_
	 */
	...
	public function isPHP($ver){
		if( $ver == 4 )
			...
			return $this->php_ver === PhpVersion::$php7;
		else
	...
	public function getConstant($name){
		return cast(Constant::class, $this->constants->get($name));
	}
	...
	public function getFunc($name){
		return cast(Function_::class, $this->functions->get($name));
	}
	...
	public function getClass($name){
		return cast(ClassType::class, $this->classes->get($name));
	}
	...
	public function getPackage($fn) {
		return cast(Package::class, $this->packages->get($fn));
	}
	 ...
	 */
	public abstract function addSuspended($package_parser);
	 ...
	 * @return void
	 */
	public abstract function continueParse();
	...
	public static function static_init() {
		...
		self::$AUTOLOAD_FQN = new FullyQualifiedName("phplint_autoload", FALSE);
	...
	public function __construct($logger){
		...
		ClassType::initActualizedClassesCache();
		$this->classes = new HashMap();
		$this->classes->put(self::$object_type->name, self::$object_type);
		
		$this->packages = new HashMap();
		
		$this->builtin = new BuiltinClasses();
	}
	...
	public function searchConstant($name)
	{
		if( NamespaceResolver::isAbsolute($name) ){
			...
			return cast(Constant::class, $this->constants->get($fqn));
			...
			if( $this->curr_pkg->resolver->inNamespace() ){
				...
				$co = cast(Constant::class, $this->constants->get($fqn));
				if( $co !== NULL ){
...
//					return NULL;
	...
	/**
	 * Search a variable inside the current scope and then among the
	...
	public function searchVar($name)
	{
		...
		/* Next, search among superglobals: */
	...
	public function searchFunc($name)
	{
		if( NamespaceResolver::isAbsolute($name) ){
			...
			return cast(Function_::class, $this->functions->get($fqn));
			...
			if( $this->curr_pkg->resolver->inNamespace() ){
				...
				$f = cast(Function_::class, $this->functions->get($fqn));
				if( $f !== NULL ){
	...
	public function searchClass($name, $is_fqn = FALSE){
		...
		if( $is_fqn || NamespaceResolver::isAbsolute($name) ){
			if( NamespaceResolver::isAbsolute($name) )
				$name = substr($name, 1);
			$fqn = new FullyQualifiedName($name, FALSE);
			$c = cast(ClassType::class, $this->classes->get($fqn));
			if( $c === NULL )
				$c = $this->autoload($fqn);
	...
	/**
	 * Seach a class resolving its name in the current namespace, or formal type
	 * parameter. To be called from any context where a type parameter is allowed
	 * (then NOT from actual PHP code, where type parameters are not allowed).
	 * @param string $name Name as found in the source, that might need to
	 * be resolved in the current namespace context.
	 * @param boolean $is_fqn If true, assumes the name be already fully
	 * qualified or absolute and does not applies the namespace resolution
	 ...
	 * @return ClassType Resolved class, or NULL if not found.
	 */
	public function searchClassOrTypeParameter($name, $is_fqn = FALSE)
	{
		$pkg = $this->curr_pkg;
		if (
			$pkg->curr_class !== NULL
			&& $pkg->curr_class->is_template
			&& NamespaceResolver::isIdentifier($name)
			&& array_key_exists($name, $pkg->curr_class->parameters_by_name)
		){
			return $pkg->curr_class->parameters_by_name[$name];
		} else {
			return $this->searchClass($name, $is_fqn);
		}
	}
	...
	/**
	 * Attempts to perform class autoloading if the <code>phplint_autoload()</code>
