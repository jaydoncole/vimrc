 ...
 * @version $Date: 2018/07/27 10:58:56 $
 */
class Linter {
	
	const VERSION = "3.2_20180727";
...
private static function version($logger){
	...
	"Copyright 2018 by icosaedro.it di Umberto Salsi\n" .
...
"  --help                  this help\n" .
"  --php-version V         set PHP version V (either 5 (default) or 7)\n" .
...
"\nReport bugs to salsi@icosaedro.it (but read README before submit).\n" .
...
public static function main($os, $argv)
{
	...
	$packages_to_document = /*. (File[int]) .*/ array();
	$do_report = FALSE; // generate report only if at least 1 package parsed
	$report_prototypes = FALSE;
	...
	for($i = 1; $i < count($argv); $i++){
		...
		case "--report-prototypes"   : $report_prototypes = TRUE;  break;
		case "--no-report-prototypes": $report_prototypes = FALSE;  break;
		...
		case "--php-version":
			...
			if( $argv[$i] === "4" ){
				$logger->error(NULL, "phplint: PHP 4 support dropped");
			} else if( $argv[$i] === "5" ){
				$globals->php_ver = PhpVersion::$php5;
			} else if( $argv[$i] === "7" ){
				$globals->php_ver = PhpVersion::$php7;
			} else {
				$logger->error(NULL, "phplint: invalid PHP version - must be either 5 or 7");
				exit(1);
			}
			break;
		...
		default:
			if( Strings::startsWith($arg, "--doc") ){
				...
				if( ($doc !== NULL || $report_prototypes) && $globals->getPackage($fn) !== NULL )
					$packages_to_document[] = $fn;
				$do_report = TRUE;
			}
	...
	// Resume parsing of suspended packages:
	$globals->continueParse();
	
	// Generate docs now that all resolved:
	foreach($packages_to_document as $fn){
		if( $doc !== NULL ){
			$doc->generate($fn);
		}
		if( $report_prototypes ){
			$proto = new \it\icosaedro\lint\documentator\PrototypesReporter($globals, $os);
			$proto->report($fn);
		}
	}
