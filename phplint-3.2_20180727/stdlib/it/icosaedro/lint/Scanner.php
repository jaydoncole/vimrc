 ...
 * @version $Date: 2018/06/01 14:27:55 $
 */
class Scanner {
 ...
 * @return string Last char read, or null at EOF or IOException.
 */
private function readCh()
{
	...
	if( $this->line === NULL ){ // either EOF or got IOException
		...
		$this->line_idx++;
		return $this->c = $this->line[$this->line_idx - 1];
	} else {
		try {
			...
			return $this->c = NULL; // prevent scanner from keep asking for chars
		}
		$this->line_idx = 1;
...
/**
 * Return the current byte and any following ASCII letters. An attempt to optimize
 * scanning time avoiding to many calls to the quite expensive readCh() method.
 * Prerequisite: the current byte must be a valid ID char.
 * Scanning stops when either the end of the current line or a non-ASCII letter
 * has been found. The status of the scanner is set to the last byte of the
 * returned string, then reader must call readCh() again in order to detect if
 * the end of the current ID or some non-ASCII letter is present in the ID.
 * @return string Current byte and following ASCII letters.
 */
private function readAsciiId() {
	$start = $this->line_idx - 1;
	$end = $start + 1;
	$line = $this->line;
...
public static function static_init(){
	...
	self::$php_keywords = array(
		...
		"numeric" => Symbol::$sym_unimplemented_keyword,
		"new" => Symbol::$sym_new,
		"null" => Symbol::$sym_null,
	...
	self::$phplint_keywords = array(
		...
		"if_php_ver_7" => Symbol::$sym_x_if_php_ver_7,
...
private function parseSingleQuotedString(){
	...
	do {
		...
		/* Detect ctrl chars: */
		if( $report_ctrl && self::isCtrl($c)
		&& ( $this->here_doc_id === NULL || ! self::isSpace($c) ) ){
			$this->reportCTRL($c);
			$report_ctrl = FALSE;
		}

		if( $report_ascii_ext && ! self::isAscii($c) ){
			$this->reportASCIIExt($c, "literal string");
		...
		} else if( $this->here_doc_id === NULL && $c === "'" ){
			$this->readCh();
			return $b;
...
private function parseEscapeCode(){
	if( $this->c === "n"  ){
		...
		if( $this->php_ver !== PhpVersion::$php7 )
			$this->logger->error($this->here(), "codepoint escape sequence allowed in PHP 7 only");
		$this->readCh();
		if( $this->c !== "{" )
			$this->logger->error($this->here(), "missing { after \\u");
		$this->readCh();
		$x = 0;
		do {
			if( ! self::isHex($this->c) ){
				$this->logger->error($this->here(), "invalid hexadecimal digit");
				return NULL;
			}
			$x = 16*$x + self::hexValue($this->c);
			$this->readCh();
		...
		$this->readCh();
		if( !(is_int($x) && 0 <= $x && $x <= UTF8::MAX_CODEPOINT) ){
			$this->logger->error($this->here(), "invalid range codepoint");
			return NULL;
		}
		return UTF8::chr($x);
	} else {
		$this->logger->warning($this->here(), "invalid escape sequence. Hint: allowed escape sequences are only \\n \\r \\t \\v \\f \\$ \\\" \\\\ \\0-\\377 (octal) \\x0-\\xff (hexadecimal) \\u{HEX} (PHP7)");
		return "\\" . $this->c;
	}
}
...
private function parseKeyword(){
	...
	$c = $this->c;
	do {
		if( $report_ascii_ext && ord($c) >= 127 ){
//		if( $report_ascii_ext && ! self::isAscii($c) ){
			$this->reportASCIIExt($c, "identifier");
			$report_ascii_ext = FALSE;
		}
//		$b .= $c;
		$b .= $this->readAsciiId();
...
private function parseXCode(){
	...
	case  ".":
		...
		if( $this->c === "*" ){
			...
			$this->readCh();
			$this->sym = Symbol::$sym_x_ellipsis;
			break;
	...
	case  "?":
		$this->readCh();
		$this->sym = Symbol::$sym_x_question;
		break;
	default:
		$case_found = FALSE;
	}
...
private function parseCode(){
	...
	public function readSym() {
		// If we are skipping symbols from a section of code reserved for a
		// PHP version which differs from the current one; these sections of
		// specific code are enclosed between if_php_ver_* and end_if_php_ver.
		$skip_section_for_specific_php_ver = FALSE;
		
		do {

			// If the symbol just found must be skipped and the whole read logic
			...
			switch ($this->part) {
				...
				case self::inside_code:
					$skip_symbol = $this->parseCode();
					break;

				case self::inside_x_code:
					$skip_symbol = $this->parseXCode();
					break;
				...
				case self::inside_double_quoted_string:
					 ...
					 */
					$skip_symbol = $this->parseDoubleQuotedString();
					break;
			...
			if( $this->sym === Symbol::$sym_x_if_php_ver_5 ){
				$skip_section_for_specific_php_ver = $this->php_ver !== PhpVersion::$php5;
				$skip_symbol = TRUE;
			} else if( $this->sym === Symbol::$sym_x_if_php_ver_7 ){
				$skip_section_for_specific_php_ver = $this->php_ver !== PhpVersion::$php7;
				$skip_symbol = TRUE;
			} else if( $this->sym === Symbol::$sym_x_end_if_php_ver ){
				$skip_section_for_specific_php_ver = FALSE;
				$skip_symbol = TRUE;
			}
			
		} while ($skip_symbol || $skip_section_for_specific_php_ver);
	}
