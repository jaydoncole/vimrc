 ...
 * @version $Date: 2018/04/07 11:35:15 $
 */
class Documentator {
...
"\n"
	...
	private function writeClassType($c){
		...
		if( $c->is_anonymous ){
			...
			// Class wildcard "?".
			if( $c->parentClassOf !== NULL ){
				$this->write("? parent ");
				$this->writeClassType($c->parentClassOf);
			} else {
				if( phplint_count($c->implemented) > 0 )
...
//					$this->write("?");
		...
		if( phplint_count($c->parameters_by_index) > 0 ){
			...
			$this->write("&lt;");
			for($i = 0; $i < phplint_count($c->parameters_by_index); $i++){
				if( $i > 0 )
					$this->write(",");
	...
	private function writeDocBlock($db, $contextual_class){
		...
		if( self::nonEmptyEntryFound($db->triggers_descrs) ){
			...
			for($i = 0; $i < phplint_count($db->triggers_names); $i++){
		...
		if( self::nonEmptyEntryFound($db->throws_descrs) ){
			...
			for($i = 0; $i < phplint_count($db->throws_exceptions); $i++){
		...
		for($i = 0; $i < phplint_count($db->others); $i += 2){
	...
	private function docFooter(){
		...
		if( phplint_count($modules) == 0 )
			$this->write("<i>none</i>");
		for($i = 0; $i < phplint_count($modules); $i++){
			if( $i > 0 )  $this->write(",\n");
	...
	private function writePrototype($name, $title, $sign){
		...
		for($i = 0; $i < phplint_count($sign->arguments); $i++){
			if( $i > 0 )
				$this->write(", ");
		...
		if( $sign->variadic !== NULL ){
			if( phplint_count($sign->arguments) > 0 )
				$this->write(", ");
	...
	private function docClassHierarchyRecurse($c, $indent_this, $indent_next, $anchor)
	{
		...
		for( $i = 0; $i < phplint_count($c->implemented); $i++){
			if( $i == phplint_count($c->implemented)-1 ){
				$a = "  `--";
				$b = "     ";
			} else {
				$a = "  +--";
	...
	private function docClassConstants($c){
		if( phplint_count($c->constants) == 0 )
			return;
	...
	private function docProperties($c){
		if( phplint_count($c->properties) == 0 )
			return;
	...
	private function docOverridesImplements($m){
		...
		if( phplint_count($m->class_->implemented) > 0 ){
	...
	private function docMethods($c){
		// Collect all non-private methods:
		...
		// Non-private constructor first:
		if( $c->constructor !== NULL
		&& $c->constructor->visibility !== Visibility::$private_ )
			$this->docMethod($c->constructor);
		
		// Non-private destructor second (destructors should not be private
		// anyway, but... who knows?):
		if( $c->destructor !== NULL
		&& $c->destructor->visibility !== Visibility::$private_ )
			$this->docMethod($c->destructor);
		
		// Other non private methods next:
	...
	private function docInherited($c)
	{
		...
		if( phplint_count($ms) > 0 ){
			$this->write("<hr><b>Inherited methods:</b>\n");
			for($i = 0; $i < phplint_count($ms); $i++){
				if( $i > 0 )  $this->write(",\n");
				$m = $ms[$i];
	...
	private function docClassPrivateMembers(){
		...
		if( phplint_count($this->private_class_constants) > 0 ){
			$this->write("<hr>\n");
		...
		if( phplint_count($this->private_properties) > 0 ){
			$this->write("<hr>\n");
		...
		if( phplint_count($this->private_methods) > 0 ){
			$this->write("<hr>\n");
		...
		if( $c->is_template ){
			...
			for($i = 0; $i < phplint_count($c->parameters_by_index); $i++){
				...
				if( phplint_count($bounds) > 0 ){
					$this->write(" extends ");
					for($j = 0; $j < phplint_count($bounds); $j++){
						if( $j > 0 )
							$this->write(" &amp; ");
...
//		}
	...
	private function docPrivateItems(){
		...
		if( phplint_count($this->private_constants)
		+ phplint_count($this->private_variables)
		+ phplint_count($this->private_functions)
		+ phplint_count($this->private_classes) == 0 )
			return;
		
		$this->write("<hr><h3>Private items</h3>\n");
		
		if( phplint_count($this->private_constants) > 0 ){
