 ...
 * @version $Date: 2018/04/07 11:35:22 $
 */
class Inheritance {
	...
	public static function inheritedConstants($c)
	{
		...
		if( $c->implemented !== NULL )
			foreach($c->implemented as $iface)
				self::inheritedConstantsRecurse($iface, $res);
		return cast(ClassConstant::class."[int]", Arrays::sort($res));
	}
	...
	public static function inheritedProperties($c)
	{
		...
		return cast(ClassProperty::class."[int]", Arrays::sort($res));
	}
...
//			if( $mm !== $c->constructor )
		...
		$res = cast(ClassMethod::class."[int]", $established_methods->getElements());
		foreach($abstract_methods as $m)
			if( $m !== NULL )
				$res[] = $m;
		
		return cast(ClassMethod::class."[int]", Arrays::sort($res));
	}
