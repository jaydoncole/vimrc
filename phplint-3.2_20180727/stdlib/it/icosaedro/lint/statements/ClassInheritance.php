 ...
 * @version $Date: 2018/03/05 13:51:12 $
 */
class ClassInheritance {
	...
	public static function addInterfaceToClass($c, $iface)
	{
		...
		if( $err === "" )
			...
			return "redundant interface(s)$err already implemented by $iface."
			. " PHP allows BaseInterface, DerivedInterface in that order, but gives"
			. " fatal error if the order is reversed (see PHP Bug #63816, #71358)."
			. " PHPLint is more conservative and requires a minimal set of interfaces.";
	}
	
	
	private static function hasTwoOrMoreParents(/*. ClassType .*/ $c)
	{
		$n = $c->implemented === NULL? 0 : count($c->implemented);
		if( $n > 1 )
			return TRUE;
	 ...
	 * togheter, that is must have the same exact prototype.
