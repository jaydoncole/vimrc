 ...
 * 
 * For simplicity, most of the methods return {@link it\icosaedro\sql\SQLException}
 * for both actual errors related to the client/server communication and for
 * invalid parameters submitted as well.
 * 
...
abstract class ResultSet implements Printable {
	...
	/**
	 * Maps column names into field column index. Since there might be column
	 * names collisions, collect all column numbers that share the same name
	 * so we may throw exception if the client ask for these ambiguous column
	 * names.
	 * @var int[string][int]
	 */
	private $column_name_to_indeces;
	...
	/**
	 * Returns the column index given its name.
	...
	{
		if( $this->column_name_to_indeces === NULL ){
			// Lazy init.
			$this->column_name_to_indeces = array();
			$ncols = $this->getColumnCount();
			for($i = 0; $i < $ncols; $i++){
				$name = $this->getColumnName($i);
				$this->column_name_to_indeces[$name][] = $i;
			}
		}
		if( array_key_exists($column_name, $this->column_name_to_indeces) ){
			$a = $this->column_name_to_indeces[$column_name];
			if( count($a) == 1 ){
				return $a[0];
			} else {
				$colliding = "" . $a[0];
				for($i = 1; $i < count($a); $i++)
					$colliding .= ", " . $a[$i];
				throw new SQLException("ambiguous column name `$column_name' shared by columns $colliding");
			}
		} else {
			$available = "";
			foreach($this->column_name_to_indeces as $k => $v)
				$available .= " $k";
			throw new SQLException("no this column: $column_name; available columns:$available");
		}
	}
	...
	/**
	 * Decodes literal boolean value.
	 * @param string $v
	 * @return boolean
	 * @throws SQLException
	 */
	private static function stringToBool($v)
	{
		if( $v === NULL )
			return FALSE;
	...
	/**
	 * Returns a field value from the current row as a floating point number.
	 * @param int $column_index Index of the column, starting from 0.
	 * @return float Value of the field, possibly also INF and NAN.
	 * Returns 0.0 for SQL NULL: you may use the wasNull() method to check for
	 * this latter case.
	 * @throws SQLException Failed to retrieve data from SQL server.
	 * Invalid format of the float number.
	 */
	...
	{
		$s = (string) str_replace(" ", "T", $s) . "+00:00";
		return DateTimeTZ::parse($s);
	}


	/**
	 * Returns a field value from the current row as a gregorian date and time.
	 * @param int $column_index Index of the column, starting from 0.
	 * @return DateTimeTZ Value of the field. Returns PHP NULL for SQL NULL.
	 * @throws SQLException Failed to retrieve data from SQL server.
	 * Invalid format of the date.
	 */
	function getDateTimeByIndex($column_index)
	{
		$v = $this->getStringByIndex($column_index);
		if( $v === NULL )
			return NULL;
		try {
			...
			throw new SQLException($e->getMessage() . ": $v");
		}
	}
	 ...
	 * Invalid format of the date.
	 */
	function getDateTimeByName($column_name)
	{
		$v = $this->getStringByName($column_name);
		if( $v === NULL )
			return NULL;
